...
/* Third, the SearchableList component which uses both components, the Search and List components, for React's component composition and thus both components become siblings in the component tree:
 */
...
Both child components don't need to take care about it.
 You have lifted the state up to share the local state across the child components.
 Last but not least, let's use the query -- which is due to the state lifting available in the SearchableList component -- to filter the list for the List component:
 */
...
/*
The list gets filtered by the search query before it reaches the List component.
An alternative would be passing the query state as prop to the List component and the List component would apply the filter to the list itself.
How to lift State down?

In the next part, let's get to the second example: the "Archive in a List"-example.
It builds up on the previous example, but this time the List component has the extended functionality to archive an item in the list.
 Therefore, it needs to have a button to archive an item in the list identified by an unique id property of the item. First, the enhanced List component:
 */
...
/* The Search component stays untouched.
As you have seen, the previous example was extended to facilitate the archiving of items in a list.
    Now, the List component receives all the necessary properties: an handleArchive callback handler and the list, filtered by query and archivedItems.
    It only shows items filtered by the query from the Search component and items which are not archived.

You might see already the flaw which leads to lifting the state down.
The SearchableList takes care about the archiving functionality.
    However, it doesn't need the functionality itself. It only passes all the state and handler to the List component as props.
     It manages the state on behalf of the List component.No other component cares about this state: not the Search component and not the SearchableList component.
     In a scaling application, it would make sense to lift the state down to the List component, because only the List component cares about it and no other component has to manage it on the List component's behalf.
     Even though the List component becomes a stateful component afterward, it may be a step in the right direction keeping the local state maintainable in the long run.
    First, the enhanced stateful List component which takes care about the state: */
...
/*
That's how you can lift state down. It is used to keep the state only next to components that care about the state.
However, note that sometimes it may be useful to have umbrella components like the SearchableList component that manages state on other component's behalf.
It makes it easier to locate the important parts of your application which manage state.

    Let's recap both approaches. In the first example, the "Search a List"-example, the state had to be lifted up to share the query property in two child components.
 The Search component had to manipulate the state by using a callback handler, but also had to use the query to be a controlled component regarding the input field.
 On the other hand, the SearchableList component had to filter the list by using the query property on behalf of the List component.
 Another solution would have been to pass down the query property to the List component and let the component deal with the filtering itself.

 After all, the state got lifted up the component tree to share it vertically across more components.

In the second example, the "Archive in a List" - example, the state could be lifted down to keep the state maintainable in the long run.
The parent component shouldn't be concerned about state that isn't used by the parent component itself and isn't shared across multiple child components.
Because only one child component cared about the archived items, it was a good change to lift the state down to the only component which cares about the state.After all, the state got lifted down the component tree.

Both examples have been separated and styled over here by Richard Hess.In conclusion, lifting state allows you to keep your local state management maintainable.
